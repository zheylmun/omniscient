...
**Auto-start (opt-in, off by default):** when `[embedder] auto_start = true` and `base_url` is unreachable, `build_embedder` spawns a local llama.cpp server (`llama serve -hf <hf_repo> --port <port> --embedding --pooling <pooling>`, port parsed from `base_url`) via `embed::ManagedServer`, polls the endpoint until ready (or `auto_start_timeout_secs`), and ties the child's lifetime to the embedder (`kill_on_drop`). An already-reachable endpoint is always reused and never spawned over. The child's stdout is sent to `/dev/null` (stdout is reserved for MCP); its stderr is inherited so download/load progress shows. Only a **local** host is spawnable; a remote `base_url` errors.
