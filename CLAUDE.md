...
`omniscient.toml` at the repo root (see `omniscient.example.toml`): `[embedder] base_url/model/api_key` (+ `auto_start/llama_bin/hf_repo/pooling/auto_start_timeout_secs`), `[search] relevance_ratio/max_results/token_budget`, `languages`, `strip_comments`, `index_tests`, `exclude`. Defaults apply if the file is absent.

**Auto-start (opt-in, off by default):** when `[embedder] auto_start = true` and `base_url` is unreachable, `build_embedder` spawns a local llama.cpp server (`llama serve -hf <hf_repo> --port <port> --embedding --pooling <pooling> --ctx-size N --batch-size N --ubatch-size N`, port parsed from `base_url`) via `embed::ManagedServer`, polls the endpoint until ready (or `auto_start_timeout_secs`), and ties the child's lifetime to the embedder (`kill_on_drop`). **`N` = `server_ctx_size` = `max(2048, max_batch_bytes)`:** llama.cpp's defaults (ctx ~4096, ubatch 512) are far smaller than the requests we send, and a pooled embedding model must fit each sequence whole in one ubatch and within n_ctx — so an under-sized server passes the readiness probe (a tiny string) and then aborts on the first real reconcile batch (the "starts fine, silently dies mid-use" failure). A token is always ≥ 1 byte, so `max_batch_bytes` is a safe ceiling, and sizing off the same knob that bounds our batches means they can't drift. The documented manual `llama serve` command must carry these flags too. An already-reachable endpoint is always reused and never spawned over — and if `connect()` fails while something is still listening on the port (a TCP probe, `endpoint_listening`, confirms it), the real error is surfaced rather than spawning a second server that would collide on the port. The child's stdout is sent to `/dev/null` (stdout is reserved for MCP); its stderr is inherited so download/load progress shows. Only a **local** host (`localhost`/`127.0.0.1`/`::1`, not the `0.0.0.0` bind wildcard) is spawnable; a remote `base_url` errors.
