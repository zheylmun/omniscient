...
`omniscient.toml` at the repo root (see `omniscient.example.toml`): `[embedder] base_url/model` (+ `auto_start/llama_bin/hf_repo/pooling/auto_start_timeout_secs`), `[search] relevance_ratio/max_results/token_budget`, `languages`, `strip_comments`, `index_tests`, `exclude`. Defaults apply if the file is absent.
